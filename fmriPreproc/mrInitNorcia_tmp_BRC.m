...
if ispref('VISTA','defaultAnatomyPath')
	subjAnatDir = fullfile(getpref('VISTA','defaultAnatomyPath'),'anatomy',subjid);
...
[garbage,outNames] = filepartsgz(outFiles(:));
...
else                             % BRC: In this case, we want to align the whole-head to the high-res T1 (that's where it's tricky).
	% Register inplane volume to whole head, don't brain extract                     % BRC: what's the reason not to brain extract?
	...
	if replaceFileFlag( xfm2File )
		if reconOptions.betFlag
			...
			if replaceFileFlag( [brainFile,extFSL] )
				...
				runSysCmd( sprintf('bet %s %s -R',wholeHeadFile,brainFile) );                % BRC: Here, we need to change the bet function and to add fast to get the bias-field correction
                runSysCmd( sprintf('fast -I 40 -n 3 -l 10 -B %s', brainFile) );
                %runSysCmd (sprintf('mv %s %s',[brainFile '_restore'],brainFile)); %brokem,  fix filename handling for cleanup
                brainFile = [brainFile '_restore'];
			elseif reconOptions.verbose
...
% Voxel scaling to mm
[garbage,inplaneVolRes,garbage2,garbage3,Xinplane] = getVolumeInfo( inplaneFile );
