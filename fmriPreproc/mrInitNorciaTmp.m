function P=mrInitNorciaTmp(subjid,inputDir,TR)
...
if replaceFileFlag( [subjStructFile,extFSL] )
	...
	%Convert filled to nifti and threshold so WM = 1
	runSysCmd( sprintf('mri_convert %s %s%s',freesurferWmVol,subjWmFile,extFSL) );
...
if isempty(wholeHeadName)
	...
	if replaceFileFlag( xfm2File )
		...
		runSysCmd( sprintf('flirt %s -in %s -ref %s -omat %s -wmseg %s',flirtBbrOpts,regFile,subjStructFile,xfm2File,subjWmFile) );
	elseif reconOptions.verbose
...
save(fullfile(mrVistaHOMEDIR,'mrInitNorciaInfo.mat'));
...
if replaceFileFlag( mrSessionFile )
    ...
    OK = mrInit(P);
	if OK ~= 1
		error('Problem with mrInit.m')
	end
	fprintf('\n\ninitialized mrVista,\n')
elseif reconOptions.verbose
	fprintf('Skipping mrInit, but still updating mrSESSION.alignment\n')
end
