function [dat respTime condInfo] = sortOddStep(dataDir,optns,condition)
...
%            nTotalEpochs = (nEpochs-2*nPrelude)*length(condData);
           ...
           CycleLen = size(condData{iTrial}.RawTrial,1)./condData{iTrial}.NmbEpochs;
            ...
            condInfo{condIdx}.CycleLen = 3*CycleLen;
        end
        ...
        thisTrialByEpoch = reshape(thisTrialEEG,[],nEpochs,nEeg);
...
%        idx:idx+length(validEpochs)-1
        ...
        goodEpochLabel(idx:idx+length(validEpochs)-1,:) = condData{iTrial}.IsEpochOK(validEpochs,:)>0;
    ...
    try
        dat{condIdx} = zeros(size(fullData,1)*3,length(trialsToReturn),nEeg,'single');
    catch
        keyboard;
    end
    
    for iOdd = 1:length(trialsToReturn),
        
        %Oddstep doesn't make sense for correct rejects/False alarms, just
        %triple repeat the data;
        
        if usePreOddPost == true;
            thisData = [squeeze(fullData(:,trialsToReturn(iOdd)-1,:)); squeeze(fullData(:,trialsToReturn(iOdd),:)); ...
                squeeze(fullData(:,trialsToReturn(iOdd)+1,:))];
        else
            thisData = [squeeze(fullData(:,trialsToReturn(iOdd),:)); squeeze(fullData(:,trialsToReturn(iOdd),:)); ...
                squeeze(fullData(:,trialsToReturn(iOdd),:))];
        end
        
        dat{condIdx}(:,iOdd,:) = thisData;
