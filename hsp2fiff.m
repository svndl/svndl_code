function hsp2fiff( hspfile, flags )
% hsp2fiff takes a EMSE Localizer headshape file and writes out a fiff file
% with dummy data that can be used for creating a registration file
%
% Use as
%   hsp2fiff( hspfile, [options] )
%
% - hspfile is headshape file in the .hsp format
%
...
if nargin < 1
    error( me, 'Incorrect number of arguments. Usage: hsp2fiff( hspfile, [options] )' );
elseif( nargin == 3 )
...
Nx = fiducials(1,1); % distance from the ctf origin to nasion
...
epochs = ones(nsamples,data.info.nchan);
