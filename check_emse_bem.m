function [pFix pOrig] = check_emse_bem(cortexFile,iskullFile,oskullFile,scalpFile);


filenames = {cortexFile iskullFile oskullFile scalpFile};

meshtype={'cortex' 'inner_skull' 'outer_skull' 'scalp'};

for i=1:4,
    thisFile = filenames{i}

    [ vertices faces] = mesh_emse2matlab2(thisFile);

   % p.mesh.path{i}=path;
   % p.mesh.fil{i}e=[name,ext];

    p.mesh.data.meshtype{i}=meshtype{i};
%    p.mesh.data.vertices{i}={vertices(:,[2 3 1])-128};

%    p.mesh.data.vertices{i}=vertices([2 3 1],:)'-128;
%    p.mesh.data.vertices{i}=vertices([1 2 3],:)'-128;
%  p.mesh.data.faces={faces(:,[3 2 1])};
  p.mesh.data.faces{i}=faces';

  vertices = vertices';
  vertices = vertices-128;
  vertices(:,3)=-vertices(:,3);
  vertices = rz(vertices,+90,'degrees'); % default -90 **** LGA
  
%  vertices = vertices(:,[2 3 1]);  
%   vertices = vertices(:,[3 1  2]);  
  
  p.mesh.data.vertices{i} = vertices;
end
p.mesh.current =1;
pOrig = p;
pFix = p;
...
dist.three2two=5
dist.two2one=2
origin = [0 0 0];mean(p.mesh.data.vertices{1});

p = mesh_units_emse2brainstorm(p,origin);

mean(p.mesh.data.vertices{1})
p.mesh.data.vertices{1}(1,:)
pout = mesh_correct(p,[0 0 0],dist);

pout = mesh_units_brainstorm2emse(pout,origin);
...
for i=1:4,                  
 %   pFix.mesh.data.faces{i}=pFix.mesh.data.faces{i}(:,[3 2 1]);
 %   pOrig.mesh.data.faces{i}=pOrig.mesh.data.faces{i}(:,[3 2 1]);        
end


function [p] = mesh_correct(p,origin,dist,coin,cortex)
...
% $Revision: 1.1 $ $Date: 2008/03/24 23:44:55 $
...
function p = mesh_units_brainstorm2emse(p,origin)

for i=1:length(p.mesh.data.vertices)
    %Convert back to mm units
    p.mesh.data.vertices{i}=p.mesh.data.vertices{i}*1000;
    
    %Recenter mesh
    p.mesh.data.vertices{i}(:,1) =p.mesh.data.vertices{i}(:,1)+origin(1);
    p.mesh.data.vertices{i}(:,2) =p.mesh.data.vertices{i}(:,2)+origin(2);
    p.mesh.data.vertices{i}(:,3) =p.mesh.data.vertices{i}(:,3)+origin(3);
    
    
end

function p = mesh_units_emse2brainstorm(p,origin)
