...
% - datafile is the Axx_c???.mat from powerdiva
...
% - outputDir is wehrer to write the output.
%- doreg is a boolean as to whether or not to register the elecs.
...
%and convert covvariance to volts^2
...
if (exist('fiducialFile','var'))
    if (~isempty(fiducialFile)),
        ...
        else %doreg == false, so we just read in the previously made reg file
