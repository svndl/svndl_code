...
% - fiducialFIle is an optional file that contains the locations of the
%   fiducals in the MRI reference frame.
...
if nargin < 2
    error( me, 'Incorrect number of arguments. Usage: eeg_raw2fiff( elpfile, datafile, [options] )' );
elseif( nargin >= 4 )
...
% % import noise covariance matrix
 covfile = [ fullfile(path, name) '.cov' ];
 ...
 if (exist('fiducialFile','var'))
     ...
     %    trans = fiducial_coregister(elecFiducials([3 2 1],:),mriFiducials([3 2 1],:))';
      [t r] = alignFiducials(elecFiducials,mriFiducials);
      
      trans = [ r, t'; 0 0 0 1];
      
     
         [tmp transName] = fileparts(elpfile);
         save([transName '.tran'],'trans','-ascii')
     end
 end
...
%    snr = 1 ./ ( signal ./ noise' );
    ...
    fig = figure(100);
    clf;
    set(fig, 'NumberTitle', 'off', 'Name', name, 'Position', [ 100,100, 512,512 ], 'Color', [ 0 0.5 1 ] );
    ...
    if exist('trans','var')
        ...
        scatter3( mriFiducials(:,1)/1000, mriFiducials(:,2)/1000,mriFiducials(:,3)/1000, 120, 'g', 'filled' ),
        hold on;
